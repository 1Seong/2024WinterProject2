...
public class Player : MonoBehaviour
{
    ...
    public Player(Vector3 savedVelocity)
    {
    ...
    {
        NonConsum nonConsum = other.GetComponent<NonConsum>();
        if (nonConsum != null)
        {
            switch (nonConsum.type)
            {
                case NonConsum.Type.Pause:
                    if (!isPaused)
                    {
                        isPaused = true;
